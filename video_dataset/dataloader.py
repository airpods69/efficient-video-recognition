...
def setup_arg_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--train_data_root', type=str, default='/local/scratch/c_adabouei/video_analysis/dataset/kinetics-dataset/all_files/k400_targz/train',
                        help='training samples root directory')
    parser.add_argument('--val_data_root', type=str, default='/local/scratch/c_adabouei/video_analysis/dataset/kinetics-dataset/all_files/k400_targz/val',
...
def create_train_loader(args: argparse.Namespace, resume_step: int = 0) -> torch.utils.data.DataLoader:
    ...
    #divides sampler into args.num_steps arrays, each of which is 128 in size.    
    sampler = torch.cat(sampler, dim=0)[:args.num_steps * args.batch_size].view(args.num_steps, args.batch_size) 

    # it will take out batch_size_per_gpu(32) indices from each of the args.num_steps arrays and concatenate them 
